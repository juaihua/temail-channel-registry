...
import com.syswin.temail.beans.*;
...
import static com.syswin.temail.contants.RedisOptConstants.*;
...
public class ConnectionStatusServiceImpl{
    ...
    public TemailAccountStatusUpdateResponse updateStatus(TemailAccountStatusUpdateRequest temailAccountStatusUpdateRequest) {
        try {
            ...
            switch (temailAccountStatusUpdateRequest.getOptype()){
               case add:{
                   String acctKey = new StringBuilder(TEMAIL_PREFIX_ON_REDIS).append(temailAccountStatusUpdateRequest.getAccount()).toString();
                   redisTemplate.opsForHash().put(acctKey, temailChannelHashKey, temailAccountStatusUpdateRequest.getStatus());
                   String hstKey = new StringBuffer(HOST_PREFIX_ON_REDIS).append(temailAccountStatusUpdateRequest.getStatus().getHostOf()).append("-")
                           .append(temailAccountStatusUpdateRequest.getStatus().getProcessId()).append(":").append(temailAccountStatusUpdateRequest.getAccount()).toString();
    ...
    public CdtpServerOperateResponse registerOrRecorveryServer(CdtpServer cdtpServer) {
        try{
            ...
            redisTemplate.opsForList().range(OFFLINE_SERVERS,0,-1).stream()
                    .filter(c1 -> ((CdtpServer)c1).getIp().equals(cdtpServer.getIp()) && ((CdtpServer)c1).getProcessId().equals(cdtpServer.getProcessId()))
    ...
    /**
     * offLine the server state by moving {@link com.syswin.temail.beans.CdtpServer} from Online-Servers to OffLine-Servers
     * @return
     */
